...
void Main :: Operate()
{
	...
	(
		((_controller.Get_stick_Rx() == WEST) ? YES : NO),
		((_controller.Get_stick_Rx() == EAST) ? YES : NO)
	);
	
	MoveBelt::Operate
	(
		((_controller.Get_cross_x() == WEST) ? YES : NO),
		((_controller.Get_cross_x() == EAST) ? YES : NO)
	);
	
	MoveUpOrDown::Operate
	(
		((_controller.Get_cross_y() == NORTH) ? YES : NO),
		((_controller.Get_cross_y() == SOUTH) ? YES : NO)
	);
