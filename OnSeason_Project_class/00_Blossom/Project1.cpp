...
Main :: Main()
	...
	, _mem_move_up_down(Direc::CENTER_Y)
{
...
void Main :: Receive()
{
	...
	_mem_is_bumper_sensor_on[0] = Reversal(_mem_bumper_sensor[0].In());
	_mem_is_bumper_sensor_on[1] = Reversal(_mem_bumper_sensor[1].In());
	
	_mem_is_arm_sensor_on[0] = Reversal(_mem_arm_0_sensor[0].In() & _mem_arm_1_sensor[0].In());
	_mem_is_arm_sensor_on[1] = Reversal(_mem_arm_0_sensor[1].In() & _mem_arm_1_sensor[1].In());
	...
	_mem_wheel.Record_direc(_mem_controller.Get_stick_Lx());
	_mem_wheel.Record_direc(_mem_controller.Get_stick_Ly());
...
void Main :: Count_pwm()
{
	_mem_wheel.Record_pwm(_mem_wheel.Get_pwm());
...
void Main :: Move_up_down()
{
	...
	{
		case Direc::NORTH:		_mem_up_down = SIGNAL_FORWARD;	break;
		case Direc::SOUTH:		_mem_up_down = SIGNAL_REVERSE;	break;
		case Direc::CENTER_Y:	_mem_up_down = SIGNAL_BREAK;	break;
	}
}
...
void Main :: Move_arm()
{
	...
	if (_mem_controller.Push_Tri())	_is_enabled_auto = Reversal(_is_enabled_auto);
	
	if (Reversal(_is_moved_arm) & _is_enabled_auto)
	{
