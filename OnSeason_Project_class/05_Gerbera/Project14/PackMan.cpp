...
#define PWM_PACK_MAN_OVER	17
#define PWM_PACK_MAN_UNDER	17
...
PackMan::PackMan(const MotorNum _num, const UartNum _uart_num)
	...
	: _uart_motor(_uart_num)
	, _motor_pack_man_stand(_num)
{
	_motor_pack_man_stand.Set_steps(STEPS_USE);
	
	_motor_pack_man_arm[RIGHT_OVER	].Reset(IO::NUM_E, IO::BIT_4, IO::BIT_5);
	_motor_pack_man_arm[RIGHT_UNDER	].Reset(IO::NUM_F, IO::BIT_0, IO::BIT_1);
	_motor_pack_man_arm[LEFT_UNDER	].Reset(IO::NUM_E, IO::BIT_2, IO::BIT_3);
	_motor_pack_man_arm[LEFT_OVER	].Reset(IO::NUM_E, IO::BIT_6, IO::BIT_7);
	
	_limit_stand_under.Reset(IO::NUM_F, IO::BIT_2);
	_limit_stand_over.Reset( IO::NUM_F, IO::BIT_3);
	
	_limit_arm_stretch[RIGHT_OVER	].Reset(IO::NUM_F, IO::BIT_5);
	_limit_arm_shorten[RIGHT_OVER	].Reset(IO::NUM_F, IO::BIT_4);
	
	_limit_arm_stretch[RIGHT_UNDER	].Reset(IO::NUM_F, IO::BIT_6);
	_limit_arm_shorten[RIGHT_UNDER	].Reset(IO::NUM_F, IO::BIT_7);
	
	_limit_arm_stretch[LEFT_UNDER	].Reset(IO::NUM_K, IO::BIT_0);
	_limit_arm_shorten[LEFT_UNDER	].Reset(IO::NUM_K, IO::BIT_1);
	
	_limit_arm_stretch[LEFT_OVER	].Reset(IO::NUM_K, IO::BIT_2);
	_limit_arm_shorten[LEFT_OVER	].Reset(IO::NUM_K, IO::BIT_3);
...
void PackMan::Move_pack_man_arm_stretch()
{
	for (uByte i = 0; i < NUM_MOTOR_ARM; i++)
	{
		if (_limit_arm_stretch[i].Get() == YES)
		{
...
void PackMan::Move_pack_man_arm_shorten()
{
	for (uByte i = 0; i < NUM_MOTOR_ARM; i++)
	{
		if (_limit_arm_shorten[i].Get() == YES)
		{
...
void PackMan::Write()
{
	_motor_pack_man_stand.Write(_uart_motor);
...
{
	_motor_pack_man_stand.Write_clear(_uart_motor);
...
{
	_motor_pack_man_stand.Display_power(_adrs);
}
